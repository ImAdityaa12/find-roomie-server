...
export const userPreferences = pgTable(
    'user_preferences',
    {
        ...
        // Roommate preference
        genderPreference:
            genderPreferenceEnum('gender_preference').default('any'),
    ...
    (t) => [index('prefs_city_idx').on(t.city)]
);
...
export const savedListings = pgTable(
    ...
    (t) => [uniqueIndex('saved_listings_unique_idx').on(t.userId, t.listingId)]
);
...
export const reports = pgTable(
    'reports',
    {
        ...
        reportedListingId: text('reported_listing_id').references(
            () => roomListings.id,
            { onDelete: 'set null' }
        ),
...
export const roomListingsRelations = relations(
    roomListings,
    ({ one, many }) => ({
        user: one(user, {
            fields: [roomListings.userId],
            references: [user.id],
        }),
...
export const reportsRelations = relations(reports, ({ one }) => ({
    ...
    reportedListing: one(roomListings, {
        fields: [reports.reportedListingId],
...
export const compatibilityScoresRelations = relations(
    compatibilityScores,
    ({ one }) => ({
        userA: one(user, {
            fields: [compatibilityScores.userAId],
            references: [user.id],
            relationName: 'userA',
        }),
        userB: one(user, {
            fields: [compatibilityScores.userBId],
            references: [user.id],
            relationName: 'userB',
        }),
    })
