...
import type {
    ...
    ListingBody,
    ValidateHasRoomBody,
} from '@/modules/user/has-room/types';
...
function validatePreferencesBody(
    ...
    if (body.ageMin != null && body.ageMin < 18)
        return 'ageMin must be at least 18';
    if (body.ageMax != null && body.ageMax < 18)
        return 'ageMax must be at least 18';
...
function validateListingBody(body: Partial<ListingBody>): string | null {
    ...
    if (body.amenities != null) {
        if (
            ...
            body.amenities.some((a) => typeof a !== 'string')
        )
...
export function validateHasRoomBody(
    body: Partial<ValidateHasRoomBody>
): string | null {
    return (
